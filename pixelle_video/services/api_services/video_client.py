...
try:
    ...
    from .video_custom import CustomVideoClient
except ImportError:
    from video_dashscope import DashscopeVideoClient
    from video_kling import KlingVideoClient
    from video_seedance import SeedanceVideoClient
...
class VideoClient:
    ...
    def __init__(
        ...
        self._custom_video_client = None
    ...
    @property
    def custom_video_client(self):
        """Create custom video client only when a custom model is selected."""
        if self._custom_video_client is None:
            if not self._custom_api_key:
                raise RuntimeError(
                    "CUSTOM_API_KEY not set. "
                    "Configure the custom provider when using custom video models."
                )
            self._custom_video_client = CustomVideoClient(
                api_key=self._custom_api_key,
                base_url=self._custom_base_url,
                local_proxy=self._custom_local_proxy,
            )
        return self._custom_video_client
    ...
    def generate_video(
        ...
        if provider == "custom":
            return self._generate_custom_video(
                prompt,
                image_path,
                save_path,
                model,
    ...
    def _generate_custom_video(
        ...
        """通过第三方中转服务（如 new-api）生成视频"""
        logger.info(f"VideoClient: 路由至自定义中转 model={model}")
        return self.custom_video_client.generate_video(
            prompt=prompt,
            image_path=image_path,
            save_path=save_path,
            model=model,
