...
class ImageClient:
    def __init__(self,
                 ...
                 custom_local_proxy: Optional[str] = None):
        """
        Unified Image Generation Client
        Routes requests to DashScope, Seedream, GPT, or Custom based on model name / provider.
        """
        ...
        self._custom_api_key = custom_api_key or Config.CUSTOM_API_KEY
        self._custom_base_url = custom_base_url or Config.CUSTOM_BASE_URL
        self._custom_local_proxy = custom_local_proxy
    ...
    def custom_client(self):
        """Create custom image client only when a custom model is selected."""
        if not self._custom_api_key:
            raise RuntimeError("CUSTOM_API_KEY not set. Configure the custom provider when using custom image models.")
        if self._custom_client is None:
            self._custom_client = ImageGPT(
                api_key=self._custom_api_key,
                base_url=self._custom_base_url,
                local_proxy=self._custom_local_proxy,
            )
    ...
    def generate_image(self,
        ...
        if provider == "custom":
            # --- Custom Relay Logic (OpenAI-compatible) ---
            try:
                logging.info(f"ImageClient requesting Custom relay: {model}")

                gpt_size = size.replace('*', 'x') if size else "1024x1024"

                path = self.custom_client.generate_image(
                    prompt=prompt,
                    size=gpt_size,
                    model=model,
                    save_dir=save_dir,
                ...
                if path and os.path.exists(path):
                    ...
                    logging.error(f"Custom relay returned invalid path or download failed: {path}")

            except Exception as e:
                logging.error(f"Custom relay image generation failed: {e}")
                raise RuntimeError(f"Custom relay image generation failed: {e}") from e
